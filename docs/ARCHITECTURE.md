...
```
   ...
   │ RPi CM5      │◄────────────►│ VL817 hub A │◄────────────►│ ASM1153E ×2  │◄┼──► pigtail ──► bays 1, 2
   │ (eMMC/Lite,  │  USB 3.0 #1  ├─────────────┤  USB 3.0 ×2  ├──────────────┤ │  (data + 5 V + 12 V)
   │  GbE, RTC)   │◄────────────►│ VL817 hub B │◄────────────►│ ASM1153E ×2  │◄┼──► pigtail ──► bays 3, 4
   └──┬───┬───┬─┬─┘              └─────────────┘              └──────────────┘ │
...
Chosen topology: **one VL817 hub per native USB 3.0 port, two ASM1153E bridges
per hub.**

| Link | Practical rate | Shared by |
|---|---|---|
| CM5 USB 3.0 port → VL817 | ~400 MB/s | two bays |
| VL817 → ASM1153E, per port | ~400 MB/s | one bay |
...
* *One VL817 on one port for all four bays:* ~100 MB/s per bay under load, and
  the second native port idle. Cheaper by one hub, slower by ~1.5×.

Each VL817 has its own 25 MHz crystal and per-port power-enable/overcurrent
lines; those are left unused since bay power is switched on the drive rails
(§3.6), not the USB side. Bridges are always powered.
...
* **NVMe:** the CM5's PCIe Gen3 x1 lane is unused by the SATA bays, so an
  optional fifth bay, an M.2 M-key slot with its own switched 3V3 rail, can give
  native NVMe wipes (`nvme sanitize`, `nvme format --ses`). It is decision D10;
  the v1 board carries the footprint unpopulated. Caveats: PCIe is not
  hot-plug, so the slot is power-cycled and the bus rescanned per job; the
  enclosure needs an access door; and the boot order must exclude NVMe.
...
| 5V_SYS | TPS56637 buck | CM5 (≤3.0 A), 2× VL817 (0.3 A), 4× ASM1153E (0.4 A), UI (0.1 A) | 2.0 A | 3.8 A | 3.8 A |
...
6. Any 1.2 V core rails the VL817 or ASM1153E require come from small LDOs off
...
* **M.2 M-key slot (D10, DNP):** PCIe Gen3 x1, switched 3V3 at 3 A, PERST# and
  CLKREQ# from the CM5.
...
* **Stack:** 4-layer, 1.6 mm, ENIG. Sig / GND / PWR / Sig. Target the JLCPCB
  ...
  USB 3 SS and SATA pairs, 85 Ω for the optional PCIe Gen3 pair. Length-match
...
* **Fab:** JLCPCB or PCBWay, 5 pcs. VL817 and ASM1153E are 0.4–0.5 mm QFN, and
...
See `docs/DECISIONS.md`. Closed: CM5, USB topology, module size. Still open:
input connector (barrel vs DIN), bridge chip (ASM1153E vs ASM235CM), OLED size,
the optional M.2 NVMe bay, and what to build first.
