...
| Board | **v4 brain 150 × 122 mm + bay card 40 × 46 mm (C24, 2026-09-23)**: eight PCIe-x1 bay slots along the rear, hubs behind them, CM5 on the right edge, check_place clean, DRC 0 errors after the four fanout passes (brain: 103 + 2 + 62 + 30 vias). Routing chains running on both boards (stage 1 signals done; stage 2 planes / rails on 2.1.0 takes hours; stage 3 pairs on 2.4.1). First card chain: 116 / 130 connections (89 %), DRC clean, 14 open (10 plane pins, 4 pair ends) before the net-class fix; numbers land in `routing/open.md` and `routing/pairs.md` of each project when the chains finish. Previous monolithic v3: 83 % | `hardware/renders/`, `hardware/routing/`, `hardware/bay-card/routing/` |
