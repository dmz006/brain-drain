...
| Board | **v4 brain 150 × 122 mm + bay card 40 × 46 mm (C24, 2026-09-23)**: eight PCIe-x1 bay slots along the rear, hubs behind them, CM5 on the right edge, check_place clean, DRC 0 errors after the four fanout passes (brain: 103 + 2 + 62 + 30 vias). Routing chains running on both boards (stage 1 signals done; stage 2 planes / rails on 2.1.0 takes hours; stage 3 pairs on 2.4.1). **Bay card routed: all but 6 connections (2 plane pins, 1 LED, 3 differential-pair ends), 0 DRC errors** after the finger fanout and P-FET pin fix; pairs unmatched (`bay-card/routing/pairs.md`); numbers land in `routing/open.md` and `routing/pairs.md` of each project when the chains finish. Previous monolithic v3: 83 % | `hardware/renders/`, `hardware/routing/`, `hardware/bay-card/routing/` |
...
| R24 | Choose the bay-card P-FET part (DFN 3x3-8, −30 V, ≥ 6 A, pin-out S 1–3 / G 4 / D 5–8 + EP) and download its datasheet; confirm the AO4407A pin table for Q20 against the AOS datasheet | owner | the symbols assume the standard power pin-out |
