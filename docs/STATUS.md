...
| Board | **v4 brain 150 × 122 mm + bay card 40 × 46 mm (C24)**. **Brain, six layers (C26) with the hubs' bypass caps on the bottom, after the pairs-first chain, gap closer and tuner: 2 connections open of about 800** (the feedback pin of the U21 buck converter, and one 5V_HDD inner-layer island note), DRC 0 errors, all 44 pairs routed and 43 matched (one bay pair is 13 mm short with no room to meander); the whole flow runs in about 85 minutes (`routing/open.md`, `routing/pairs.md`). **Bay card: 1 open** (the ASM1153E REXT resistor pin, which cannot escape between its neighbours), 0 DRC errors, all 8 pairs routed and matched (`bay-card/routing/`). Q20 and the card FETs use the corrected 8-pin symbols. Previous monolithic v3: 83 % | `hardware/renders/`, `hardware/routing/`, `hardware/bay-card/routing/` |
