...
| Board | **v2 outline 136 × 100 mm (C21–C23)**, bay rows re-placed (switch block behind each receptacle's power pads, bridge behind its data pads), CM5IO design rules (0.13 / 0.125 mm, 0.45/0.2 vias; power tracks 0.3 mm). **Automated routing, 2026-09-23: 719 of 884 pad-to-pad connections (81 %)**: 3175 track segments (9.0 m), 658 vias; every single-ended signal net complete; DRC 0 copper errors (5 thermal-relief notes). Still open, 165 connections: 75 plane pins (mostly QFN supply pins in dense spots), 25 bay power / bridge-rail links, 65 differential-pair ends. Of the 48 pairs only 9 are routed on both sides and none is length-matched (mismatch 0.5–31 mm): the autorouter does not do controlled-impedance pairs. Reports: `routing/open.md`, `routing/pairs.md` | `hardware/renders/`, `routing/open.md`, `routing/pairs.md` |
...
| R22 | Differential pairs: 39 of 48 have an unrouted side, the 9 routed ones are unmatched (`routing/pairs.md`). Needs the KiCad GUI diff-pair router and length tuning, about a day of designer time, or a placement that shortens them (bridges closer to their receptacles' data pads already; hubs closer to the CM5 USB 3 pins) | R4 | |
