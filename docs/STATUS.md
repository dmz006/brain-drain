...
| Board | **v4 brain 150 × 122 mm + bay card 40 × 46 mm (C24)**. **Brain, after the full chain: 43 connections open** (25 plane pins on 5V_SYS, +3V3, 5V_HDD and others, 18 differential-pair ends), 2629 track segments (11.4 m), 525 vias, DRC 0 copper errors (2 starved-thermal notes); 44 pairs, 36 beyond the 0.15 mm match, 7 pair reports list an unrouted side (`routing/open.md`, `routing/pairs.md`). **Bay card: 6 open** (2 plane pins, 1 LED return, 3 pair ends), 0 DRC errors (`bay-card/routing/`). Q20 and the card FETs use the corrected 8-pin symbols. Previous monolithic v3: 83 % | `hardware/renders/`, `hardware/routing/`, `hardware/bay-card/routing/` |
