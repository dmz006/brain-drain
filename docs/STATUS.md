...
| Board | **v4 brain 150 × 122 mm + bay card 45 × 46 mm (C24, C27)**. **Brain, six layers (C26), real PCIe slot footprint, hubs' bypass caps on the bottom, pair coupling caps side by side, after the pairs-first chain, gap closer and tuner: no open signal connections, DRC 0 errors, all 44 pairs routed and length-matched.** Known plane defect: slot 1's two 5V contacts (A8, A9) sit inside the +12V corner polygon on In4 and get only tiny 5V islands (DRC lists two zone fragments); the fix (a +12V strip across the slot row instead of the corner polygon) is in `route.py _zones` and needs a routing rerun (`routing/open.md`, `routing/pairs.md`). **Bay card (45 × 46 mm, real Molex receptacle and fingers): 0 open, 0 DRC errors, all 8 pairs routed and matched, bridge rotated 180 degrees at x 38 (best of ten placements tried) (`bay-card/routing/`). Q20 and the card FETs use the corrected 8-pin symbols. Previous monolithic v3: 83 % | `hardware/renders/`, `hardware/routing/`, `hardware/bay-card/routing/` |
