...
| Board | **v4 brain 150 × 122 mm + bay card 40 × 46 mm (C24)**. **Brain, after the pairs-first chain, gap closer and tuner: 9 connections open of about 800** (2 plane pins, 3 single-ended signals that are long or sit in tight pin escapes, 4 differential-pair ends into the CM5 and the second hub), DRC 0 copper errors (2 starved-thermal notes), 41 of 44 pairs matched (`routing/open.md`, `routing/pairs.md`). **Bay card: 1 open** (the ASM1153E REXT resistor pin, which cannot escape between its neighbours), 0 DRC errors, all 8 pairs routed and matched (`bay-card/routing/`). Q20 and the card FETs use the corrected 8-pin symbols. Previous monolithic v3: 83 % | `hardware/renders/`, `hardware/routing/`, `hardware/bay-card/routing/` |
