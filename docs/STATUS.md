...
| Board | **v4 brain 150 × 122 mm + bay card 40 × 46 mm (C24)**. **Brain, with the hubs' bypass caps on the bottom side, after the pairs-first chain, gap closer and tuner: 10 connections open of about 800** (1 inner-plane island of 5V_HDD, the fan tach signal, and 8 differential-pair ends: four bay pairs to slots 3, 5, 6 and 8, two USB 3 lines from the second hub to the CM5, PCIE_CLK_N to the M.2, and one hub coupling cap), DRC 0 copper errors (2 starved-thermal notes), 43 of 44 pairs matched; the supply-pin opens went from 25 to none (`routing/open.md`, `routing/pairs.md`). **Bay card: 1 open** (the ASM1153E REXT resistor pin, which cannot escape between its neighbours), 0 DRC errors, all 8 pairs routed and matched (`bay-card/routing/`). Q20 and the card FETs use the corrected 8-pin symbols. Previous monolithic v3: 83 % | `hardware/renders/`, `hardware/routing/`, `hardware/bay-card/routing/` |
