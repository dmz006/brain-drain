"""Bay 5 (M.2 NVMe slot): door, PEDET, slot power, abort on door open."""
...
def test_slot_stays_dark_with_door_open(runner):
    cfg = runner.cfg
    plug(cfg.sim_dir, 5, 1024 * 1024, media="nvme", serial="NVME1", firmware={"crypto": True, "seconds": 0.3})
    time.sleep(0.6)
    assert runner.engine.bays[5].state == "IDLE"
    assert not runner.engine.bay_power.is_on(5)
    assert "door open" in (cfg.sim_dir / "display.txt").read_text()


def test_door_closed_powers_slot_and_wipes_nvme(runner):
    cfg = runner.cfg
    plug(cfg.sim_dir, 5, 1024 * 1024, media="nvme", serial="NVME1", prefill=True,
         firmware={"crypto": True, "seconds": 0.3})
    door(cfg, "closed")
    runner.wait_state(5, "DONE", 20)
    assert runner.engine.bay_power.is_on(5)
    c = read(certs(cfg)[0])
...
def test_door_open_mid_wipe_aborts_and_powers_off(runner):
    cfg = runner.cfg
    plug(cfg.sim_dir, 5, 16 * 1024 * 1024, media="nvme", serial="SLOWNV", throttle_bps=4 * 1024 * 1024)
    (cfg.sim_dir / "dip").write_text("01000000\n")  # CLEAR: host overwrite so it takes a while
    door(cfg, "closed")
    runner.wait_state(5, "RUNNING", 10)
    time.sleep(0.4)
    door(cfg, "open")
    runner.wait_state(5, "IDLE", 10)
    c = read(certs(cfg)[0])
...
def test_sata_module_is_refused(runner):
    ...
    plug(cfg.sim_dir, 5, 1024 * 1024, media="nvme", serial="SATAM2")
    door(cfg, "closed")
    time.sleep(0.8)
    assert runner.engine.bays[5].state == "IDLE"
    assert not runner.engine.bay_power.is_on(5)
