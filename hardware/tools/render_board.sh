...
cd "$(dirname "$0")/.."
mkdir -p renders
kicad-cli pcb export pdf -l F.Cu,F.SilkS,F.Mask,Edge.Cuts,F.CrtYd -o renders/board-top.pdf brain-drain.kicad_pcb
kicad-cli pcb export pdf -l In1.Cu,In2.Cu,B.Cu,Edge.Cuts -o renders/board-inner.pdf brain-drain.kicad_pcb
...
"$(dirname "$0")/../../software/.venv/bin/python" - <<'PY'
